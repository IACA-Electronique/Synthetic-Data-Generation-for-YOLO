...
impl ImageGenerator {
    ...
    pub fn generate<I: EditableImage>(
        &self,
        background_dir: &str,
        object_dir: &str,
        distraction_dir: Option<&str>,
        output_path: &str,
    ) -> Result<(), String> {
        let mut rng = rand::thread_rng();

        let backgrounds = self.filesystem.list_files(background_dir)?;
        if backgrounds.is_empty() {
            return Err(format!("No background images found in {}", background_dir));
        }
        ...
        if num_objects > 0 {
            let all_objects = self.get_all_files_in_subdirs(object_dir)?;
            if all_objects.is_empty() {
                return Err(format!("No object images found in {}", object_dir));
            }
        ...
        if let Some(dir) = distraction_dir {
